...
require_deployment_token() {
  [[ -n "${DEPLOYMENT_TOKEN:-}" ]] || {
    printf "DEPLOYMENT_TOKEN environment variable is required\\n" >&2
    exit 1
  }
}
...
get_deployment_info() {
  ...
  local result
  result=$(PGPASSWORD="$DB_PASS" psql -U "$DB_USER" -d "$DB_NAME" -h "$DB_HOST" -t -c "$query" | tr -d '[:space:]')
...
main() {
  ...
  local name version config
  name=$(jq -r .project_name <<< "$json")
  version=$(jq -r .version <<< "$json")
  config=$(jq -r .config <<< "$json" | yq -P)

  log "🚀 Starting deployment for $name version $version"
  deploy_with_helm "$name" "$config"
  log "✅ Deployment of $name version $version completed"
}
