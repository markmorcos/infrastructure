...
# Apply forward DB migrations from INSIDE the cluster, via a one-shot Job.
#
# Why not psql-on-the-runner (scripts/migrate.sh)? That reads DATABASE_URL and
# connects from the GitHub Actions runner, which is OUTSIDE the cluster — so it
# can't use in-cluster names (the `data` ExternalName aliases). This runs psql in
# a Job in the target namespace instead: cluster DNS resolves, DB creds never
# leave the cluster, and the runner only needs kubectl (no psql, no accept-dns).
#
# Each *.sql MUST be idempotent (re-applied every deploy). Files run in filename
# order, each in a single transaction; the first failure aborts the Job.
#
# Env:
